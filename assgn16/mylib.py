...
# Contains classes for solving ODEs:
#   - ForwardExplicitEuler
#   - PredictorCorrector
...
class ForwardEulerSolver:
    def __init__(self, func, x0, y0, h, N):
        # Store ODE, range, and step size
        self.f = func
        self.x0 = x0
        self.y0 = y0
        self.h = h
    ...
    def solve(self):
        data = [(self.x0, self.y0)]
        x, y = self.x0, self.y0
        for _ in range(self.N):
            y = y + self.h * self.f(x, y)
            x += self.h
            data.append((x, y))
        return data

    # Write stepwise data and summary
    def write_output(self, process_file, output_file, exact_func, title):
        data = self.solve()
        with open(process_file, "a") as pf:
            pf.write(f"\n--- Forward Euler ({title}) ---\n")
            pf.write("i\tx\t\ty\n")
            for i, (x, y) in enumerate(data):
                pf.write(f"{i}\t{x:.6f}\t{y:.6f}\n")

        y_exact = exact_func(data[-1][0])
        error = abs(y_exact - data[-1][1])
        with open(output_file, "a") as of:
            of.write(f"\nForward Euler ({title}):\nFinal y = {data[-1][1]:.6f}, "
                     f"Exact = {y_exact:.6f}, Error = {error:.2e}\n")
        return data
...
class PredictorCorrectorSolver:
    ...
    def solve(self):
        ...
        x, y = self.x0, self.y0
        for _ in range(self.N):
            yp = y + self.h * self.f(x, y)                       # Predictor
            y = y + (self.h / 2) * (self.f(x, y) + self.f(x + self.h, yp))  # Corrector
            x += self.h
            data.append((x, y))
        return data
    ...
    def write_output(self, process_file, output_file, exact_func, title):
        ...
        error = abs(y_exact - data[-1][1])
        with open(output_file, "a") as of:
            of.write(f"Predictor-Corrector ({title}):\nFinal y = {data[-1][1]:.6f}, "
                     f"Exact = {y_exact:.6f}, Error = {error:.2e}\n")
        return data
...
class SimpsonRK4:
    def __init__(self, func, a, b, exact_value=None, name=""):
        self.f = func
        self.a = a
        self.b = b
        self.exact = exact_value
    ...
    # Perform Simpson integration for a given N
    def integrate(self, N):
        if N % 2 == 1:
            N += 1
        a, b = self.a, self.b
    ...
    def compute_and_write(self, output_file, process_file, N):
        result = self.integrate(N)
        error = abs(result - self.exact) if self.exact is not None else None

        with open(output_file, "a") as f_out, open(process_file, "a") as f_proc:
            f_out.write("\n" + "="*70 + f"\n{self.name} Integration (N={N})\n" + "="*70 + "\n")
            f_out.write(f"N = {N}\nApproximation = {result:.9f}\n")
            if self.exact is not None:
                f_out.write(f"Exact = {self.exact:.9f}\nError = {error:.2e}\n")
...
# General RK4 Solver for n coupled ODEs

class RK4Integrator:
    def __init__(self, func, name="RK4 Solver"):
        self.func = func
        self.name = name

    # Integrate from x0 to x_end with step h, write outputs
    def integrate_and_write(self, y0, x0, x_end, h, output_file, process_file):
        X_vals = [x0]
        Y_vals = [y0[:]]
        x = x0
        y = y0[:]
        ...
        with open(output_file, "a") as f_out, open(process_file, "a") as f_proc:
            f_out.write("\n" + "="*70 + f"\n{self.name} Integration (h={h})\n" + "="*70 + "\n")
            f_proc.write(f"\n--- RK4 Steps ({self.name}), h={h} ---\n")
            f_proc.write("Step\tX\tY-values\n")

            step = 0
            while x < x_end - 1e-10:
                k1 = self.func(y, x)
                k2 = self.func([y[i] + 0.5*h*k1[i] for i in range(len(y))], x + 0.5*h)
                k3 = self.func([y[i] + 0.5*h*k2[i] for i in range(len(y))], x + 0.5*h)
                k4 = self.func([y[i] + h*k3[i] for i in range(len(y))], x + h)
                ...
                f_proc.write(f"{step}\t{x:.6f}\t" + "\t".join(f"{val:.6f}" for val in y) + "\n")
                step += 1

            f_out.write(f"Total Steps = {step}\nFinal X = {X_vals[-1]:.6f}\nFinal Y = {[round(v,6) for v in Y_vals[-1]]}\n")
...
# ---------------------------
# Shooting solver for 2nd-order BVP using RK4Integrator
# ---------------------------
class ShootingSolver:
    """
    Solve y'' + alpha*(Ta - y) = 0 with y(0)=T0, y(L)=T_L using shooting (secant).
    Uses RK4Integrator to integrate first-order system.
    """

    def __init__(self, alpha, Ta, L, T0, TL, integrator=None):
        self.alpha = alpha
        self.Ta = Ta
        self.L = L
        self.T0 = T0
        self.TL = TL
        self.integrator = integrator if integrator is not None else RK4Integrator(self._system, name="Shooting RK4")

    # system for RK4: y = [T, T']
    def _system(self, y, x):
        T = y[0]
        Tp = y[1]
        # T' = Tp, Tp' = -alpha*(Ta - T)
        return [Tp, -self.alpha * (self.Ta - T)]

    # integrate from 0 to L with initial slope s
    def integrate_with_slope(self, s, h, output_file, process_file):
        y0 = [self.T0, s]
        X, Y = self.integrator.integrate_and_write(y0, 0.0, self.L, h, output_file, process_file)
        T_vals = [yy[0] for yy in Y]
        return X, T_vals

    # secant method to find slope s so that T(L)=TL
    def solve(self, h, output_file, process_file, tol=1e-6, max_iter=50):
        # initial slope guesses
        s0 = -1.0
        s1 = 1.0

        # compute F(s) = T(L; s) - TL
        def F(s):
            X, T = self.integrate_with_slope(s, h, output_file, process_file)
            return T[-1], X, T
        ...
        it = 0
        s = s1
        Xlast, Tlast = None, None

        # secant loop
        while abs(f1) > tol and it < max_iter:
            if (f1 - f0) == 0:
                break
            s = s1 - f1 * (s1 - s0) / (f1 - f0)
            T_Ls, Xs, Ts = F(s)
        ...
        # final integrate once more to capture final arrays (avoid double write by integrator? it's okay)
        X_final, T_final = Xlast, Tlast
        if X_final is None:
            X_final, T_final = self.integrate_with_slope(s, h, output_file, process_file)

        return s, X_final, T_final

    # find x where T(x) crosses a value (linear interpolation)
    @staticmethod
    def find_x_for_temperature(X, T_vals, target):
        for i in range(1, len(X)):
            if (T_vals[i-1] - target) * (T_vals[i] - target) <= 0:
                # linear interpolate between i-1 and i
                x1, x2 = X[i-1], X[i]
                y1, y2 = T_vals[i-1], T_vals[i]
                if y2 == y1:
                    return x1
    ...
    # produce and save plot (with RK4 points as dots)
    @staticmethod
    def plot_solution(X, T_vals, target_temp, savefile="shooting_solution.png"):
        plt.figure(figsize=(8,4.5))
        plt.plot(X, T_vals, '-', label='T(x) RK4')
        plt.scatter(X, T_vals, s=12, marker='o')  # show RK4 calculation points
        x_target = ShootingSolver.find_x_for_temperature(X, T_vals, target_temp)
        if x_target is not None:
            plt.axvline(x_target, color='red', linestyle='--', label=f"T={target_temp} at x={x_target:.4f}")
        plt.xlabel("Position, x")
        plt.ylabel("Temperature (°C)")
        plt.title("Shooting solution of T'' + alpha (Ta - T) = 0")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
...
# 1D Heat equation solver (FTCS explicit)
class HeatEquationSolver:
    """
    Solve u_t = u_xx on x in [0, L] with u(0)=u(L)=0 and initial condition
    where the center node is set to peak_value and others to 0.
    Explicit FTCS method: u_i^{n+1} = u_i^n + r*(u_{i+1}^n - 2u_i^n + u_{i-1}^n)
    """

    def __init__(self, L=2.0, nx=100, dt=1e-4, t_final=0.02, peak_value=300.0):
        self.L = L
        self.nx = nx
        self.dx = L / nx
        self.dt = dt
        ...
        self.r = dt / (self.dx ** 2)
        # small safety: warn if unstable-ish (we don't print here, user can check output file)
        # boundary nodes are kept at 0.

    # evolve solution and return snapshots list of (t, x_list, u_list)
    def solve_and_record(self, output_file, process_file, snapshot_times=None):
        if snapshot_times is None:
            snapshot_times = [0.0, self.t_final*0.05, self.t_final*0.1, self.t_final*0.25, self.t_final*0.5, self.t_final]

        # grid points: 0..nx
        x = [i * self.dx for i in range(self.nx + 1)]
        # initial condition: all zero except the center node
        u = [0.0 for _ in x]
        center_index = int(round( (self.nx/2) ))
        if center_index < 0: center_index = 0
        if center_index > self.nx: center_index = self.nx
        u[center_index] = self.peak_value

        snapshots = []
        current_time = 0.0

        # write initial state to process file
        with open(process_file, "a") as pf:
            pf.write("\n--- Heat equation FTCS solve ---\n")
            pf.write(f"L={self.L}, nx={self.nx}, dx={self.dx:.6g}, dt={self.dt:.6g}, r={self.r:.6g}, t_final={self.t_final}\n")
            pf.write("time\tcenter_val\n")
            pf.write(f"{current_time:.6f}\t{u[center_index]:.6f}\n")

        # record initial snapshot if required
        if 0.0 in snapshot_times:
            snapshots.append((0.0, list(x), list(u)))

        # time-stepping
        steps = int(math.ceil(self.t_final / self.dt))
        for n in range(1, steps + 1):
            u_new = [0.0 for _ in x]  # boundaries remain 0
            # interior nodes
            for i in range(1, self.nx):
                u_new[i] = u[i] + self.r * (u[i+1] - 2*u[i] + u[i-1])
            u = u_new
            current_time += self.dt

            # if we passed any snapshot_times, record (use <= to catch small dt mismatches)
            for ts in snapshot_times:
                if abs(current_time - ts) < 0.5*self.dt:
                    snapshots.append((current_time, list(x), list(u)))
            # also occasionally store some intermediate times
            if (n % max(1, steps//10) == 0) and (current_time not in [s[0] for s in snapshots]):
                snapshots.append((current_time, list(x), list(u)))

            # log center temperature every  (small)
            if n % max(1, steps//20) == 0:
                with open(process_file, "a") as pf:
                    pf.write(f"{current_time:.6f}\t{u[center_index]:.6f}\n")

        # ensure final is recorded
        if snapshots == [] or abs(snapshots[-1][0] - current_time) > 1e-12:
            snapshots.append((current_time, list(x), list(u)))

        # write summary to output file
        with open(output_file, "a") as of:
            of.write("\nHeat equation (FTCS) summary:\n")
            of.write(f"nx = {self.nx}, dx = {self.dx:.6g}, dt = {self.dt:.6g}, r = {self.r:.6g}\n")
            of.write(f"Total steps = {steps}, Final time = {current_time:.6g}\n")
            of.write(f"Center index = {center_index}, initial peak = {self.peak_value}\n")

        return snapshots

    # plot snapshots (several profiles) to file
    @staticmethod
    def plot_snapshots(snapshots, savefile="heat_equation_profiles.png"):
        plt.figure(figsize=(8,4.5))
        for (t, x, u) in snapshots:
            # label times with limited decimals
            plt.plot(x, u, label=f"t={t:.4g}")
        plt.xlabel("Position, x")
        plt.ylabel("Temperature (°C)")
        plt.title("Heat equation u_t = u_xx (FTCS)")
...
class LagrangeSolver:
    ...
    def interpolate(self, x0):
        """
        Perform Lagrange interpolation for a given x0.
        Returns interpolated y(x0) and detailed computation steps.
        """
        n = len(self.x)
        y0 = 0.0
        details = []
        for i in range(n):
            ...
            for j in range(n):
                if i != j:
                    Li *= (x0 - self.x[j]) / (self.x[i] - self.x[j])
            contrib = self.y[i] * Li
            y0 += contrib
            details.append(f"Term {i+1}: L_{i}({x0})={Li:.6f}, Contribution={contrib:.6f}")
        details.append(f"Final interpolated y({x0})={y0:.6f}")
        return y0, details

    def plot(self, x0, filename="problem1.png"):
        """
        Plot interpolation curve and save as PNG.
        """
        xs, ys = [], []
        for k in range(int(min(self.x)) - 1, int(max(self.x)) + 2):
            yk, _ = self.interpolate(k)
            xs.append(k)
            ys.append(yk)

        plt.figure(figsize=(7, 5))
        plt.plot(self.x, self.y, 'ro', label="Data points")
        plt.plot(xs, ys, 'b-', label="Interpolated curve")
        y0, _ = self.interpolate(x0)
        plt.scatter(x0, y0, color='g', label=f"y({x0})={y0:.3f}")
        plt.xlabel("x")
        plt.ylabel("y")
...
# -------------------------------------------------------
# Curve Fitting Class (Power & Exponential Models)
# -------------------------------------------------------
class CurveFitter:
    def __init__(self, x, y):
        self.x = x
    ...
    # ---------- General Pearson r^2 function ----------
    def _pearson_r2(self, X, Y):
    #    Compute Pearson correlation coefficient r^2 for two data sets X, Y.
    #        Sxx = Σ (Xi - X̄)^2
    #        Syy = Σ (Yi - Ȳ)^2
      #      Sxy = Σ (Xi - X̄)(Yi - Ȳ)
     #       r^2 = Sxy^2 / (Sxx * Syy)
        ...
        Sxx = sum((X[i] - xmean) ** 2 for i in range(n))
        Syy = sum((Y[i] - ymean) ** 2 for i in range(n))
        Sxy = sum((X[i] - xmean) * (Y[i] - ymean) for i in range(n))

        # r^2 in [0, 1]
        r2 = (Sxy ** 2) / (Sxx * Syy)

        return r2, Sxx, Sxy, xmean, ymean

    # ---------- General linear fit using Pearson r^2 ----------
    def _linear_fit(self, X, Y):
        """
        Linear regression:  Y = c + m X

        Uses _pearson_r2 to get Sxx, Sxy, means and r^2.
        """
        n = len(X)
        r2, Sxx, Sxy, xmean, ymean = self._pearson_r2(X, Y)

        # slope and intercept
        m = Sxy / Sxx
        c = ymean - m * xmean

        # residuals and sigma in Y-space
        y_pred = [c + m * xi for xi in X]
    ...
    def fit_power(self):
        """
        Fit: y = a * x^b
        Linearization: Y = ln(y), X = ln(x), Y = ln(a) + b X
        """
        ln_x = [math.log(xi) for xi in self.x]
        ln_y = [math.log(yi) for yi in self.y]

        b, ln_a, r2, sigma = self._linear_fit(ln_x, ln_y)
        a = math.exp(ln_a)

        # single-line detail format
        details = [f"a={a:.6f}, b={b:.6f}, r2={r2:.6f}, sigma={sigma:.6f}"]
        return a, b, r2, sigma, details

    # ---------- Exponential Fit ----------
    def fit_exponential(self):
        """
        Fit: y = a * exp(-b x)
        Linearization: Y = ln(y), X = x, Y = ln(a) - b X
        """
        ln_y = [math.log(yi) for yi in self.y]
        X = self.x

        m, ln_a, r2, sigma = self._linear_fit(X, ln_y)
        b = -m
        a = math.exp(ln_a)
    ...
    # ---------- Plotting ----------
    def plot_power(self, filename="problem2_power.png"):
        a, b, _, _, _ = self.fit_power()
        xs = [i for i in range(1, 25)]
        ys = [a * (x ** b) for x in xs]
        plt.figure()
        plt.plot(self.x, self.y, 'ro', label="Data points")
        plt.plot(xs, ys, 'b-', label="Power fit")
        plt.xlabel("x")
        plt.ylabel("y")
    ...
    def plot_exponential(self, filename="problem2_exp.png"):
        a, b, _, _, _ = self.fit_exponential()
        xs = [i for i in range(1, 25)]
        ys = [a * math.exp(-b * x) for x in xs]
        plt.figure()
        plt.plot(self.x, self.y, 'ro', label="Data points")
        plt.plot(xs, ys, 'g-', label="Exponential fit")
        plt.xlabel("x")
        plt.ylabel("y")
